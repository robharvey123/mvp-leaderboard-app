  ...
  return (
    <div className="min-h-[60vh] grid place-items-center p-6">
      <div className="w-full max-w-md rounded-2xl border border-gray-200 bg-white p-6 shadow">
        ...
        {msg && <div className="mb-3 rounded-xl border border-emerald-200 bg-emerald-50 text-emerald-700 px-3 py-2 text-sm">{msg}</div>}

        <form onSubmit={onSubmit} className="space-y-3">
          <label className="block text-sm">
            <span className="text-gray-600">Name</span>
            <input
              type="text" placeholder="Alex Batter"
        ...
        <div className="mt-4 text-sm text-gray-600">
          ...
          <Link to="/auth/sign-in" className="text-[rgb(var(--brand-700))] hover:underline">Sign in</Link>
        </div>
      </div>
    </div>
  );
